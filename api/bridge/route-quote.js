// api/bridge/route-quote.js (fixed for current cache.js signature)
...
export default async function handler(req, res) {
  ...
  try {
    await cacheSet(quote_id, { corridor, receiver_key: destination.receiver_key, settlement_mode }, 600);
    console.log("Quote cached:", quote_id);
  } catch (err) {
    console.error("Cache error while saving quote:", err);
  }
